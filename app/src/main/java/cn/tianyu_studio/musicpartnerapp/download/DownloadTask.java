...
public class DownloadTask extends AsyncTask<String, Integer, Integer> {
    ...
    protected Integer doInBackground(String... params) {
        if (params[1].equals(".jpg")) {
            String downloadUrl = params[0];
            String format = params[1];
            int flag = TYPE_SUCCESS;  //如果第一张失败就不会继续下载
            for (int i = 0; i < downloadUrl.split(",").length; i++) {
                if (flag == TYPE_SUCCESS)
                    flag = doDownload(downloadUrl.split(",")[i], format);
    ...
    private Integer doDownload(String... params) {
        InputStream is = null;
        ...
        try {
            ...
            file = new File(directory + fileName + format);
            if (file.exists()) {
